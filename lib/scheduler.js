/*jshint node:true */

var AWS = require('aws-sdk');
var http = require('http');
var s3 = new AWS.S3();
var ecs = new AWS.ECS();
var sqs = new AWS.SQS();

var s3BucketName = 'ember-twiddle-addons-beta';
var schedulerLambdaFunctionname = 'ember-twiddle-addon-build-scheduler';
var sqsQueueName = 'beta-addon-builds';
var sqsQueueUrl = 'https://sqs.us-east-1.amazonaws.com/620471542343/beta-addon-builds';

exports.scheduleBuild = function scheduleBuild(event, context) {
  var addon;
  var addonVersion;
  var emberVersion;

  if ( 'addon' in event ) {
    addon = event.addon;
    addonVersion = event.addon_version;
    emberVersion = event.ember_version;
  }

  pushBuildToSQS(emberVersion, addon, addonVersion)
    .then(getNextBuildFromSQS, function() {
      context.done('No more builds in SQS');
    })
    .then(createSTSTokenForBuild)
    .then(runECSTask)
    .catch();
};

function pushBuildToSQS(emberVersion, addon, addonVersion) {
  return new Promise(function(resolve, reject) {
    var params = {
      QueueUrl: sqsQueueUrl,
      MessageBody: JSON.stringify({
        addon: addon,
        addon_version: addonVersion,
        ember_version: emberVersion
      })
    };

    sqs.sendMessage(params, function(err, data) {
      if (err) {
        return reject(err);
      }
      return resolve();
    });
  });
}

function getNextBuildFromSQS() {
  return new Promise(function(resolve, reject) {
    var params = {
      QueueUrl: sqsQueueUrl,
      MaxNumberOfMessages: 0,
      VisibilityTimeout: 5,
      WaitTimeSeconds: 0
    };

    sqs.receiveMessage(params, function(err, data) {
      if (data && data.length) {
        var build = JSON.parse(data[0].Body);
        resolve(build.ember_version, build.addon, build.addonVersion);
      }
    });
  });
}

function createSTSTokenForBuild(emberVersion, addon, addonVersion) {
  return new Promise(function(resolve, reject) {
    var buildAddonPolicy = {
      "Version": "2012-10-17",
      "Statement": [
          {
              "Effect": "Allow",
              "Action": [
                  "s3:GetBucketCORS",
                  "s3:GetBucketLocation",
                  "s3:GetBucketLogging",
                  "s3:GetBucketNotification",
                  "s3:GetBucketPolicy",
                  "s3:GetBucketRequestPayment",
                  "s3:GetBucketTagging",
                  "s3:GetBucketVersioning",
                  "s3:GetBucketWebsite",
                  "s3:GetLifecycleConfiguration",
                  "s3:ListBucket"
              ],
              "Resource": [
                  "arn:aws:s3:::ember-twiddle-addons-beta"
              ]
          },
          {
              "Effect": "Allow",
              "Action": [
                  "s3:PutObject",
                  "s3:PutObjectAcl",
                  "s3:GetObject",
                  "s3:GetObjectAcl",
                  "s3:GetObjectTorrent",
                  "s3:GetObjectVersion",
                  "s3:GetObjectVersionAcl",
                  "s3:GetObjectVersionTorrent"
              ],
              "Resource": [
                  "arn:aws:s3:::ember-twiddle-addons-beta/ember-"+emberVersion+"/"+addon+"/"+addonVersion+"/*"
              ]
          }
      ]
    };

    var stsParams = {
      RoleArn: 'arn:aws:iam::620471542343:role/addon-builder-role', /* required */
      RoleSessionName: 'build-addon', /* required */
      DurationSeconds: 900,
      Policy: JSON.stringify(buildAddonPolicy)
    };

    sts.assumeRole(stsParams, function (err, data) {
      if (err) {
        return reject(err);
      }

      return resolve(emberVersion, addon, addonVersion, data);
    });
  });
}

function runBuild(emberVersion, addon, addonVersion, stsParams) {
  return new Promise(function(resolve, reject) {
    var params = {
      taskDefinition: 'addon-builder', /* required */
      cluster: 'ember-twiddle',
      count: 1,
      overrides: {
        containerOverrides: [
          {
            name: 'addon-builder',
            environment: [
              {
                name: 'AWS_ACCESS_KEY_ID',
                value: data.Credentials.AccessKeyId
              },
              {
                name: 'AWS_SECRET_ACCESS_KEY',
                value: data.Credentials.SecretAccessKey
              },
              {
                name: 'AWS_SESSION_TOKEN',
                value: data.Credentials.SessionToken
              },
              {
                name: 'ADDON_NAME',
                value: addon
              },
              {
                name: 'ADDON_VERSION',
                value: addonVersion
              },
            ]
          }
        ]
      },
      startedBy: 'api_call'
    };
    ecs.runTask(params, function(err, data) {
      if (err)  {
        return reject(err);
      }
      return resolve();
    });
  });
}

